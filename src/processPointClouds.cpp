...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
{
    ...
    typename pcl::CropBox<PointT> crop(false);
    crop.setMax(maxPoint);
    crop.setMin(minPoint);
    crop.setInputCloud(cloudFiltered);
    crop.filter(*cloudFiltered);

    std::vector<int> indices;
    typename pcl::CropBox<PointT> roof(true);
    crop.setMax(Eigen::Vector4f(1.5, 0.75, 0.5, 1.0));
    crop.setMin(Eigen::Vector4f(-1.5, -0.75, -0.5, 1.0));
    crop.setInputCloud(cloudFiltered);
    crop.filter(indices);
    ...
    std::cout << "Number of points in the cloud after filtering:  " << cloudFiltered->points.size () << std::endl;
